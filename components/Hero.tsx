...
const cards = [
  ...
  },
  {
    ...
    desc: `Use ${COMPANY.shortName}'s free AI-powered software to design, sell and manage solar projects.`,
    cta: `Explore ${COMPANY.shortName}`,
    href: "/signup",
  ...
  },
  {
    ...
    desc: `Partner with ${COMPANY.shortName} to present your products and services and connect with solar pros.`,
    cta: "Partner with us",
    href: "/signup",
...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-between overflow-hidden pt-24">
      ...
      {cards.map((c) => (
        <div key={c.id} className="absolute inset-0 bg-cover bg-center transition-opacity duration-700"
          style={{ backgroundImage: `url(${c.bg})`, opacity: activeCard === c.id ? 1 : 0 }} />
      ))}
      <div className="absolute inset-0 bg-cover bg-center transition-opacity duration-700"
        style={{ backgroundImage: `url(${defaultBg})`, opacity: activeCard === null ? 1 : 0 }} />
